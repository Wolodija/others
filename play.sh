#!/bin/bash
matches=$(curl --silent 'http://sport.tvp.pl/mundial2018/transmisje' | grep -oP '/mundial2018.*?-mecz-live');
...
echo ${link_live}
stream_id=$(curl $link_live --silent | grep -oP 'data-object-id="\d+"' | grep -oP '\d+' | head -n 1);
echo ${stream_id}
